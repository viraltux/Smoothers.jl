...
@testset "loess" begin
    ...
    x = fx(xv)
    @test x ≈ [0.8414709848078966, 0.9095525828909523, 0.1420295878550277, -0.7350481509902661, -0.9589242746631387]

    xv = sort(sin.(collect(1:5.)))
    fx = loess(xv,sin.(collect(1:5.)); d=1)
    x = fx(xv)
    @test x ≈ [0.8414709848078965, 0.9092974268256818, 0.14112000805986713, -0.7568024953079286, -0.9589242746631383]
